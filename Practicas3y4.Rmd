...
```{r}
rel<-gather(pew_religion, income, cases, -religion, factor_key=T)
str(rel)
...
```{r}
load('data/billboard.Rdata')
musica<-pivot_longer(data =raw,cols =wk1:wk76, names_to = 'Semana' , values_to = 'Valores',names_prefix ='wk' )
...
load('data/tb.Rdata')
raw[-c(10,17)]
tbclean<-gather(raw,key ='sexo' ,value ='casos',m014:fu,factor_key=T )

separate(tbclean,sexo,c('Sex','Age'),sep=1)
#filter(!is.na(cases))
```
...
raw_tidy <- raw %>% 
  pivot_longer(cols =d1:d31 ,names_to ='dia' ,values_to ='valores',names_prefix = 'd', values_drop_na = TRUE) %>%
  pivot_wider(names_from = element, values_from =valores)
...
#Flights muestra información del vuelo, mientras que planes son los datos del avión
```
...
planes.f<-planes%>%
  ...
  #filter(str_detect(str_to_lower(manufacturer),'.douglas.*',negate=T))
...
```{r }
flights.f<-flights%>%
  inner_join(planes.f,by='tailnum') #unir dos dataframes por una columna
...
```{r }
flights<-flights %>% mutate(Full_Date= paste(flights$day, flights$month, flights$year.x, sep = "-"))
flights$Full_Date<-as.Date(flights$Full_Date)
...
agregado<-flights%>%
  group_by(tailnum)%>%
  summarize(Num_flights=n(),Mean_distance=mean(distance),Total_distance=sum(distance))
```
...
canciones<-raw %>% select(artist,track,time) %>% unique() %>% mutate(id = row_number())  

ranking<-raw %>% left_join(canciones %>% select(track, id), by = "track") %>% select(id, week, rank)   
...
table5_tidy<-table5 %>% separate(rate,into=c("cases", "population"))
table5_tidy<-table5_tidy %>% mutate(year = paste(century, year, sep = ""))
table5_tidy<-table5_tidy[-2]
```
